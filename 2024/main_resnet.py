...
from dataset_utils import getCIFAR10, getSTL10, do_fl_partitioning, get_dataset_double, get_dataloader_double, \
    get_dataloader
...
class Net(nn.Module):
    def __init__(self) -> None:
        ...
        # self.load_state_dict(torch.load("resnet_model.pt"), strict=False)
        # self.load_state_dict(torch.load("trained_resnet_model.pt"), strict=False)
...
# borrowed from Pytorch quickstart example
def train(net, trainloader, epochs, device: str, rnd, cid=None):
    """Train the network on the training set."""
    client_loss = 0.0
    criterion = torch.nn.CrossEntropyLoss()
    lr = 0.01 * ((0.998) ** (float(rnd) - 1))
    ...
    for _ in range(epochs):
        for images, labels in trainloader:
            ...
            client_loss += loss
        scheduler.step()
    print(f'cid {cid} loss {client_loss}')
    return client_loss
...
class CifarRayClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        trainloader = get_dataloader(
            self.fed_dir, self.cid, is_train=True, batch_size=int(config["batch_size"]), workers=num_workers
        )
        ...
        # train
        client_loss = train(self.net, trainloader, epochs=int(config["epochs"]), device=self.device,
                            rnd=config["epoch_global"], cid=self.cid)
...
def get_eval_fn(
    ...
    def evaluate(weights: fl.common.Weights) -> Optional[Tuple[float, float]]:
        ...
        model.to(device)
        torch.save(model, './trained_resnet_model.pt')
...
if __name__ == "__main__":
    ...
    # print("##############################################")
    # test_trainloader = get_dataset_double(
    #     Path(fed_dir),
    #     Path(fed_dir2),
    #     "0",
    #     "train",
    # )
    # img, target = test_trainloader.data2[0], int(test_trainloader.targets2[0])
    ...
    with open('0.01_0.9.json', 'w') as f:
